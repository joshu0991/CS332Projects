	...
	private void setUpEmptyGrid(){
		...
		for(int i = 0; i < columns; i++){
			if(i == 0){
				...
				prevTop = top; //no longer the first colum track previous column
				temp = new Node(); 
				t_counter += 1;
			...
			for(int j = 0; j < rows -1; j++){
				...
				if(j == 7){ //last time though the loop and last node so link it to the begining
					newNode.bottom = top;
				}
				
				if(prevTop != null){//if it is null this is the first column else need to link to the right 
					prevTop = prevTop.bottom;
					prevTop.right = newNode;
				}
				if(i == 4){//link the last column back to the beginning
					Node tmp = head.bottom;
					for(int k = 0; k <= j; k++){//find the node by traversing downward in the first column
						tmp = tmp.bottom;
					}
